...
戻り値 `unresolvedFindings` の `[VERIFY-UNCERTAIN]` 項目（workflow 内の検証 agent の rawLine 出力不備 — 不存在ではない。retro-e4）は、この `test -s` の結果で**置換**する: 全件 OK なら Checkpoint 提示から除外し「証跡 N 件をオーケストレータが実在確認済み（workflow 内の機械検証は出力不備で未確定）」と 1 行で記す。MISSING が混じればその分を `[BLOCKER]` へ昇格する。
