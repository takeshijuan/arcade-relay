...
rawLine 不一致の扱い（v0.5.1.0 / retro-e4）: 不一致は「未検証」であって「不存在」ではない — E4 で haiku が basename で `ls -l` を実行し 78/78 件が擬陽性となり、QA-PLAY APPROVE が CONCERNS に誤降格した。workflow は是正指示付きで 1 回だけ再検証（`verify-evidence-*-recheck`。対象は不一致分のみ）し、それでも不一致なら verdict を降格せず `[VERIFY-UNCERTAIN]` として unresolvedFindings に載せる。降格するのは不存在・0 バイト・checks に現れない・evidencePaths 空・目視未実施のみ。rawLine は `<path> <bytes>`（失敗時 `<path> MISSING`）として**解析**し、exists/nonEmpty の申告と矛盾すれば生出力を採る（`<path> MISSING` = 不存在、サイズ 0 = 0 バイト — パス文字列を含むだけの行を合格にしない）。初回で確定した不存在は再検証で消えない（fail closed マージ — batch-verify の resolvedPrior と同じ原則）。オーケストレータは `[VERIFY-UNCERTAIN]` を、その行が名指しするパスに対する自分の `test -s` 結果で置換する（§3）。
