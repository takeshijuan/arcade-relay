...
- **最小フェイク Scene のドリフト** — `src/ui/**` のテストは実 Phaser.Game を使わず「そのクラスが呼ぶ Scene 表面だけ」を満たすフェイク（`scene.add.text` 等）で代替している。後続 story が同じクラスに新しい Scene 表面（`scene.add.rectangle` / `scene.tweens.add` / `setDepth` 等のチェーンメソッド）を追加すると、**production の型は `as never` キャストで通るためコンパイルは成功し、テストだけが `TypeError: ... is not a function` で落ちる**。UI クラスに新しい Scene API 呼び出しを足したら、同名の `*.test.ts` のフェイク Scene も同じコミットで追随更新する（既存の `src/ui/shopWindow.test.ts` の `fakeRectangle` が定型）
...
- **art レーンとの並走で `PENDING_ASSET_KEYS` 解除が3箇所セットになる** — gameplay レーンが art 資産の到着を待たず実装する際、未生成スプライトシートを `bootAssetManifest.ts` の `PENDING_ASSET_KEYS` へ一時登録して安全にビルドを通す設計（S-56 で確立）は、art 資産が実際に到着した時点で (1) `PENDING_ASSET_KEYS` からキーを外す (2) `config.ts` のフレーム実寸定数（暫定値→実測値、`sips -g pixelWidth -g pixelHeight` 等で計測しフレーム数で割る）を追随更新する (3) `bootAssetManifest.test.ts` の `SPRITESHEET_DATA_URIS`（`import.meta.glob` パターン）に新しいファイル名プレフィクスを追加する、の3点セットを**同時に**やらないと `npx vitest run` が失敗する（E4 batch-verify 2026-09-04・S-53 到着時に発生）。1箇所だけ直すと別の失敗に化けるため、修正時は3点セットを一度に確認する
