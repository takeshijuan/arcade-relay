...
const GIT_ADD_RULE =
  ...
  'コミット hash は `git rev-parse HEAD` ではなく `git log --format="%H %s" -5` から**自分のコミットメッセージに一致する行**の hash を取る（rev-parse HEAD は並走レーンの直後コミットを拾い得る）。' +
...
const LANE_RULE =
  ...
  STATE.stories + ' は自 story の status 行のみをピンポイント Edit（ファイル全面書き直し禁止 — 並走レーンの更新を消す）。' +
...
async function batchVerify(phaseName, contextNote) {
  const bv = await agent(
    ...
    '4) 修正した場合は ' + STATE.reviewsDir + '/batch-verify.md に「phase / 原因 story / 修正内容 / ISO8601 日時」を追記し、git commit -m "batch-verify fix (' + phaseName + ')"。' + GIT_ADD_RULE + '\n' +
  ...
  if ((bv.fixedNotes || []).length > 0) {
    log('batch-verify(' + phaseName + '): ' + bv.fixedNotes.length + ' 件修正（' + STATE.reviewsDir + '/batch-verify.md）');
  }
  ...
  for (const story of laneStories) {
    ...
    const loopResult = await reviewLoop({
      ...
      produce: async function () {
        const r = await agent(
          [
            ...
            '   コミットメッセージ: "' + story.id + ': ' + story.title + '"。コミット hash は上記コミット規律の方法（`git log --format="%H %s" -5` の自メッセージ一致行）で取得せよ。',
            '',
      ...
      review: async function (iteration) {
        ...
        const reviewCommon = [
          ...
          '前提（並走レーン設計）: 他レーンの story が提供予定の API への参照は、' + ART.architecture + ' の設計に合致していれば「実体未実装」だけを理由に REJECT/blocker としない（コンパイル整合はレーン合流後のバッチ検証が保証する。設計との不一致・誤用は通常どおり指摘してよい）。',
      ...
      revise: async function (findings, iteration) {
        const r = await agent(
          [
            ...
            '   コミットメッセージ: "' + story.id + ': fix CR-CODE iter ' + iteration + '"。コミット hash は上記コミット規律の方法（`git log --format="%H %s" -5` の自メッセージ一致行）で取得せよ。',
    ...
    const bookkeep = await agent(
      [
        ...
        'コミットする: `git add ' + STATE.stories + '` && git commit（メッセージ: "' + story.id + ': status done"）。' + GIT_ADD_RULE,
      ].join('\n'),
...
const assetCommonRules = [
  ...
  '保存先は ' + EP.rawAssetDir + ' 配下。完了後はパス限定で add してコミット: `git add ' + EP.rawAssetDir.replace(/\/$/, '') + ' design docs state && git commit`。',
  ...
  if (round < QA_MAX) {
    ...
    for (const bug of qaResult.criticalBugs) {
      const fixed = await agent(
        ...
        { label: 'fix-qa-' + bug.assignee, phase: 'QA', agentType: bug.assignee, effort: 'high' }
      );
      if (fixed === null) {
    ...
    if ((qaResult.failedAcceptance || []).length > 0) {
      const faFixed = await agent(
        ...
        { label: 'fix-qa-acceptance', phase: 'QA', agentType: 'gameplay-engineer', effort: 'high' }
      );
      if (faFixed === null) {
