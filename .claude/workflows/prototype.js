...
const GIT_ADD_RULE =
  ...
  'コミット hash は `git rev-parse HEAD` ではなく `git log --format="%H %s" -20` から**自分のコミットメッセージに一致する最上（最新）の行**の hash を取る（rev-parse HEAD は並走レーンの直後コミットを拾い得る。一致行が窓に無ければ -50 で再取得）。' +
...
const assetCommonRules = [
  ...
  '保存先は ' + EP.rawAssetDir + ' 配下。完了後はパス限定で add し、**commit も必ずパス指定形**: `git add ' + EP.rawAssetDir.replace(/\/$/, '') + ' design docs state && git commit -m "<msg>" -- ' + EP.rawAssetDir.replace(/\/$/, '') + ' design docs state`（素の git commit は禁止 — 並走コードレーンが stage 済みのファイルを巻き取る）。',
