...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Build', detail: 'phase:prototype ストーリーを順次実装し、story ごとに CR-CODE レビューループ（MAX 2）を通す' },
...
const GIT_ADD_RULE =
  '触ったファイル/ディレクトリのみを明示的に git add（例: ' + EP.codeAddExample + '）。' +
  '`git add -A` は禁止（並走する資産生成の生成物 ' + EP.rawAssetDir + ' や design/ を巻き込まない）。' +
  ...
  for (const story of stories) {
      ...
      produce: async function () {
        const r = await agent(
          [
            ...
            '   ' + EP.placeholderNote,
            '3. ' + EP.verifyCmd + ' が exit 0 になるまで自己修正。',
            '4. ' + STATE.stories + ' で status を review に更新し、コミットする。' + GIT_ADD_RULE,
            '   コミットメッセージ: "' + story.id + ': ' + story.title + '"。コミット後 `git rev-parse HEAD` で hash を取得せよ。',
            '',
      ...
      revise: async function (findings, iteration) {
        const r = await agent(
          [
            ...
            '',
            '手順:',
            '1. 各指摘に対応（対応しない場合は正当理由を明記。黙殺禁止）。規約は ' + ART.conventions + ' / ' + DOCS.techStack + '。',
            '2. ' + EP.verifyCmd + ' が exit 0 を確認。',
            '3. ' + reviewLogPath + ' の iteration ' + iteration + ' の「対応:」欄に対応済み/見送り＋理由を追記。',
            '4. コミットする。' + GIT_ADD_RULE,
            '   コミットメッセージ: "' + story.id + ': fix CR-CODE iter ' + iteration + '"。コミット後 `git rev-parse HEAD` で hash を取得せよ。',
    ...
    const bookkeep = await agent(
      [
        ...
        '併せて ' + STATE.active + ' の現在地を「' + story.id + ' 完了」に更新。',
        'コミットする: `git add ' + STATE.stories + ' ' + STATE.active + '` && git commit（メッセージ: "' + story.id + ': status done"）。' + GIT_ADD_RULE,
      ].join('\n'),
