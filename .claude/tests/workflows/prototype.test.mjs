...
test('Setup 環境検証: engine=unity は背景のみの acceptance を不合格にし「地面」を要求する', async () => {
  const bgOnly = {
    ...SETUP,
    prototypeStories: SETUP.prototypeStories.map((st) =>
      st.id === 'S-04' ? { ...st, acceptance: '背景・ライト・カメラ構図が確定している' } : st), // 地面の言及なし（contract §11: 3D は可視の地面必須）
  };
  const routes = [
  ...
  const fix = promptsBy(calls, /^setup-fix-required-stories$/);
  assert.equal(fix.length, 1, '背景のみの acceptance が engine=unity で差し戻されない');
  assert.ok(fix[0].includes('必須環境要素を欠く: 地面'), '欠落要素（地面）が差し戻し文言に列挙されない');
});
